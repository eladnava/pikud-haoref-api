...
module.exports = function (callback, options) {
    ...
    co(function* () {
        try {
            ...
            for (let lang in config.hfc.website.translatedZonesJson) {
                // Fetch translated accurate zone names
                var translatedZones = yield extractCitiesFromUrl(config.hfc.website.translatedZonesJson[lang], options, lang);

                // Traverse results
                for (let translatedCityId in translatedZones) {
                    // Find Hebrew city obj
                    for (let hebrewCityId in cities) {
                        // Match?
                        if (translatedCityId === hebrewCityId) {
                            ...
                            if (lang === 'he') {
                                // Update zone
                                cities[hebrewCityId].zone = translatedZones[translatedCityId].zone;
                            }
                            else {
                                // Update translated zone name
                                cities[hebrewCityId]['zone_' + lang] = translatedZones[translatedCityId].zone;
                            }
            ...
            for (let cityId in cities) {
                ...
                var result = {
                    id: city.id || parseInt(city.cityId),
                    name: city.name,
                };
                ...
                if (!city.zone) {
                    console.log('No zone for city, skipping: ' + city.name);
                    continue;
                }
                ...
                for (let lang in config.hfc.website.translatedCitiesJson) {
                    ...
                    if (city['zone_' + lang]) {
                        ...
                        // Have zone in Hebrew but not translated?
                        if (city.zone) {
                            // Check archive
                            if (city.archive['zone_' + lang]) {
                                // Fallback to archive (for Hebrew & translation)
                                result['zone'] = city.archive.zone;
                    ...
                    if (!city['translation_' + lang]) {
                        ...
                        // Still don't have a translated zone name for this city?
                        if (!result['zone_' + lang]) {
                            // Extract from translation_lang object
                ...
                result.countdown = city.countdown != undefined ? city.countdown : yield getCountdownForCity(city, options);
...
function* extractCitiesFromUrl(url, options, lang) {
    // Execute GET request
    ...
    for (var city of cities) {
        ...
        // Log missing area
        if (!city.areaname) {
            // Fetch zone name from old cities archive based on city name (if exists)
            for (var cityArchive of archive) {
                // Check for match
                if (cityArchive.name === city.name) {
                    // Return zone based on language
                    city.areaname = (!lang || lang === 'he') ? cityArchive.zone : cityArchive['zone_' + lang];
                }
            }
        ...
        // Skip old cities
        if (city.name.includes('אזור התרעה ישן')) {
            console.log('Skipping old city: ' + city.name);
            continue;
        }

        // Remove region prefix/suffix
        city.areaname = city.areaname.replace('אזור ', '');
        city.areaname = city.areaname.replace('Golan Katzrin', 'Katzrin');
        ...
        if (city.areaname === 'Samaria') 
            city.areaname = 'Shomron';

        if (city.areaname === 'West Lakhish') 
            city.areaname = 'West Lachish';
...
function* getCountdownForCity(city, options) {
    ...
    try {
        // Try parsing and fail gracefully
        result = JSON.parse(json.body);
    } catch (e) {
        // Log failure and default to 0 seconds
        console.log('Unexpected countdown response for city (defaulting to 0): ' + city.name);
        return 0;
    }
    ...
    var timeIdentifier = timeIdentifiers[result.notes[0].time_notes];

    // No match?
    if (!timeIdentifier) {
        console.log('Unexpected time identifier (defaulting to 0): ' + result.time_notes);
        return 0;
    }
