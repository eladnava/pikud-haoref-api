    ...
    co(function* () {
        try {
            ...
            for (var cityId in citiesHeb) {
                ...
                for (let cityArchive of archive) {
                ...
                if (!cityHeb.lat || !cityHeb.lng) {
                    ...
                    let location = yield geocodeCity(cityHeb, options);
...
function* fetchCountdown(city, options) {
    ...
    var html = yield request('https://www.oref.org.il/Shared/Ajax/GetInstructionsByArea.aspx?lang=he&RiFid=1087&from=1&cityid=' + city.cityId + '&streetid=&ctlabel=' + encodeURIComponent(city.name) + '&stlabel=&house=&lat=0&lng=0&label=' + encodeURIComponent(city.name) + '&area=' + city.areaId + '&address=' + encodeURIComponent(city.label), options);
