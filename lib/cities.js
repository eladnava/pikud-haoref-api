...
module.exports = function (callback, options) {
    ...
    co(function* () {
        try {
            ...
            var citiesHeb = yield extractCitiesFromUrl(config.hfc.website.hebUrl);
            var citiesEng = yield extractCitiesFromUrl(config.hfc.website.engUrl);
...
function* getAreasDictionary(areas) {
    ...
    for (var area of areas) {
        ...
        // Remove time to impact from label
...
function* geocodeCity(city, options) {
    ...
    var json = yield request({ url: 'https://maps.googleapis.com/maps/api/geocode/json?components=locality:' + encodeURIComponent(city.name) + '|country:il&key=' + options.googleMapsApiKey, json: true });
    ...
    return json.body.results[0].geometry.location;
}
