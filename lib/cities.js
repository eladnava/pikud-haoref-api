...
function* getCountdownForCity(city, options) {
    // Workaround for weird HTML response when querying for גבעת ברנר
    if (city.name === 'גבעת ברנר') {
        return timeIdentifiers['דקה וחצי'];
    }


    // Workaround for weird HTML response when querying for אשדוד - איזור תעשייה צפוני
    if (city.name === 'אשדוד - איזור תעשייה צפוני') {
        return timeIdentifiers['45 שניות'];
    }

    // Execute GET request
    var json = yield request(config.hfc.website.cityNotesJson + city.value, options);
    ...
    // Get countdown object
    var countdown = timeIdentifiers[result.time_notes];
