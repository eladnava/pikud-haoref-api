...
module.exports = function (callback, options) {
    // Execute HTTP request to HFC API and get current alerts as JS array
    getHFCAlertsJSON(options, function (err, json) {
        ...
        extractAlertCitiesFromJSON(json, function (err, alertCities) {
            ...
            // Try to fetch from secondary HFC API
            options.alertsHistoryJson = true;

            // Execute HTTP request and get current alerts as JS array
            getHFCAlertsJSON(options, function (err, json) {
                // JSON retrieval failed?
                if (err) {
                    return callback(err);
                ...
                // Extract alert cities from HFC's strange JSON structure
                extractAlertCitiesFromJSON(json, function (err, secondaryAlertCities) {
                    // Alert extraction failed?
                    if (err) {
                        return callback(err);
                    }
                    ...
                    // Merge both arrays
                    alertCities = alertCities.concat(secondaryAlertCities);

                    // Remove duplicates
                    alertCities = alertCities.reduce(function (a, b) { if (a.indexOf(b) < 0) a.push(b); return a; }, []);

                    // Call the callback with current rocket alert cities from both APIs
...
function extractAlertCitiesFromHistoryJSON(json, callback) {
    ...
    for (var alert of json) {
        ...
        // If more than 5 minutes passed, skip
        if (now - unix > (60 * 5)) {
            continue;
        }
